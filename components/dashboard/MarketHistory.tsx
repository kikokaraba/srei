    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-6">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-lg font-bold text-zinc-100">Porovnanie miest</h3>
          </div>

          <div className="space-y-4 max-h-[400px] overflow-y-auto">
            {data.cityStats.slice(0, 10).map((city) => (
              <div key={city.city} className="p-3 bg-zinc-800/50 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-medium text-zinc-100">{city.city}</span>
                  <span className="text-sm text-zinc-400">{city.totalSold} predaných</span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <div>
                    <span className="text-zinc-400">€/m²:</span>
                    <span className="text-zinc-100 ml-1">€{city.avgPricePerM2.toLocaleString()}</span>
                  </div>
                  <div>
                    <span className="text-zinc-400">Dní:</span>
        ...
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-6">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-lg font-bold text-zinc-100">Mesačný vývoj</h3>
          </div>
              ...
              {data.monthlyStats.map((month) => (
                <div key={month.month} className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-zinc-300">{month.month}</span>
                    <div className="flex items-center gap-4 text-sm">
                      <span className="text-zinc-400">{month.count} predajov</span>
                      <span className="text-zinc-100">€{month.avgPricePerM2}/m²</span>
                    </div>
                  </div>
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-6">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h3 className="text-lg font-bold text-zinc-100">Posledné predané/odstránené</h3>
        </div>
              ...
              <tbody>
                {data.recentlySold.map((item) => (
                  <tr key={item.id} className="border-b border-zinc-800/50">
                    <td className="py-3">
                      <span className="text-zinc-100 line-clamp-1">{item.title}</span>
                    </td>
                    <td className="py-3 text-zinc-400">{item.city}</td>
                    ...
                    <td className={`py-3 text-right font-medium ${
                      item.priceChange < 0 ? "text-emerald-400" : item.priceChange > 0 ? "text-red-400" : "text-zinc-400"
                    }`}>
